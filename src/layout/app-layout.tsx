...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiTag, FiMusic } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'team-member', 'artist', 'music-producer'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<LabelInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/labels` : null),
    () => getLabels({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Label',
      path: isTenantUser ? `/labels/view/${data?.[0]?.id}` : '/labels',
      entity: 'label',
